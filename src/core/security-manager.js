if (typeof SecurityManager === 'undefined') {
  ...
  class SecurityManager {
    ...
    async loadSecurityModule() {
      try {
        // 檢查是否已在全域範圍載入（應該由 importScripts 載入）
        if (typeof SecurityDetectionModule !== 'undefined') {
          ...
          throw new Error('SecurityDetectionModule not available - modules should be loaded via importScripts');
        }
      } catch (error) {
    ...
    async handleSecurityCheck(details) {
      console.log(`[SecurityManager] Security check request for ${details.url} (Tab: ${details.tabId})`);
      
      // 快速檢查：模組是否可用
      if (!this.enabled) {
        console.log('[SecurityManager] Security manager is disabled, skipping check');
        return null;
      }
      
      if (this.degraded) {
        console.log('[SecurityManager] Security manager is in degraded mode, skipping check');
        return null;
      }
      
      if (!this.securityModule) {
        console.log('[SecurityManager] Security module not available, skipping check');
        return null;
      }

      // 等待初始化完成
      if (!await this.initPromise) {
        console.log('[SecurityManager] Security manager initialization failed, skipping check');
        return null;
      }

      try {
        this.stats.totalRequests++;
        console.log(`[SecurityManager] Executing security check for ${details.url}`);
        ...
        if (result) {
          ...
          console.log(`[SecurityManager] Security check completed for ${details.url}, score: ${result.score}/100`);
          
          // 確保結果被正確儲存
          if (result.tabId) {
            const storageKey = `security_tab_${result.tabId}`;
            console.log(`[SecurityManager] Result should be stored with key: ${storageKey}`);
          }
        } else {
          console.log(`[SecurityManager] Security check returned null for ${details.url}`);
        }
        
        return result;
        
      } catch (error) {
        console.error(`[SecurityManager] Error in security check for ${details.url}:`, error);
        this.handleError(error);
        return null;
      }
    }
